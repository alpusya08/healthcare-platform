...
export function AppointmentDetailModal({ appointment: appt, onClose, onCancel, onReview }: Props) {
  ...
  const isPastScheduled = appt.status === "SCHEDULED" && startDate <= now;
  const displayStatus: AppointmentStatus = isPastScheduled ? "COMPLETED" : appt.status;
  const displayLabel = isPastScheduled ? "Прошедший" : STATUS_LABELS[appt.status];
    ...
    <Dialog open={!!appt} onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-5">
          {/* Status banner */}
          <div className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-sm font-medium ${STATUS_COLORS[displayStatus]}`}>
          ...
          <div className="flex flex-col gap-2">
            ...
            {appt.status === "SCHEDULED" && !isPastScheduled && onCancel && (
              <Button
                variant="destructive"
                className="w-full"
            ...
            {(appt.status === "COMPLETED" || isPastScheduled) && !appt.hasReview && onReview && (
              <Button
                className="w-full"
            ...
            {(appt.status === "COMPLETED" || isPastScheduled) && appt.hasReview && (
              <div className="flex items-center justify-center gap-2 text-sm text-emerald-600 dark:text-emerald-400 py-1">
                <CheckCircle2 className="w-4 h-4" />
                Отзыв уже оставлен
              </div>
            )}
            {(appt.status === "COMPLETED" || appt.status === "CANCELLED" || isPastScheduled) && (
              <Button
                variant="outline"
                className="w-full"
                onClick={() => { navigate(`/doctors/${appt.doctorId}`); onClose(); }}
